...
cfonts.say('Whatsapp Bot By NzrlAfndi',{
font: 'console',
gradient: ['red','magenta'],
align: 'center'
})
...
browser: ["YT: NZRLAFNDI", "Safari", "3.0"],
auth: state
})
...
if (mek.key && mek.key.remoteJid === 'status@broadcast') return ichi.readMessages([mek.key])
...
switch (m.mtype) {
  ...
  case "senderKeyDistributionMessage":
      ...
      const quoted = m.msg ? m.msg.url : ichi.quoted.url
      if (!quoted) await ichi.refreshMediaConn(true)
      break
}
...
if (anu.action == 'add') {
var but = [{buttonId: 'menu', buttonText: {displayText: 'Welcome Brother 👋'}, type: 1},]
...
📣 Info : ${quotes.welcome}
`
ichi.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: tekswell, footer: `Welcome Message By ${global.ownerName}`, buttons: but, mentions: [num], contextInfo: { externalAdReply: { showAdAttribution: true, title: global.packname, body: global.author, description: 'Made With ❤️ '+global.ownerName, mediaType: 2, thumbnail: global.thumb, mediaUrl: global.sosmed}} })
//Message Saat Ada User Yang Keluar Dari Grup
} else if (anu.action == 'remove') {
var but = [{buttonId: 'menu', buttonText: {displayText: 'Sayonara Mayat 👋'}, type: 1},]
...
📣 Info : ${quotes.leave}
`
ichi.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: teksbye, footer: `Leave Message By ${global.ownerName}`, buttons: but, mentions: [num], contextInfo: { externalAdReply: { showAdAttribution: true, title: global.packname, body: global.author, description: 'Made With ❤️ '+global.ownerName, mediaType: 2, thumbnail: global.thumb, mediaUrl: global.sosmed}} })
...
if (update.connection == "open" || update.receivedPendingNotifications == "true") {
	...
	lolcatjs.fromString(`[ Tersambung ] ` + JSON.stringify(ichi.user, null, 2))
	let imgown = await getBuffer('https://telegra.ph/file/4ea5b7309bb948e62bc3a.jpg')
	let butown = [{urlButton: {displayText: 'Owner 👦️',url: 'https://wa.me/6287843253876?text=Assalamualaikum'}}, {urlButton: {displayText: 'Follow 📱️',url: 'https://instagram.com/oh_pahadd'}}, {urlButton: {displayText: 'Grup YFS 📺️',url: 'https://chat.whatsapp.com/HywPBmjtz45IS1wnzthJD0'}}]
	let txtown = `Halo Owner 👋, Bot Telah Berhasil Tersambung Pada Nomer Ini 🤖\n\nJika Menemukan Eror, Bug, Atau Ingin Request Fitur Silahkan Hubungi Nomer Dibawah 👇`
	lolcatjs.fromString('Sukses Mengirim Pesan Ke Owner ☑️')
    ichi.sendButtonLoc(global.Rowner+'@s.whatsapp.net', txtown, global.ownerName, imgown, butown)
	}
} catch (err) {
...
ichi.serializeM = (m) => smsg(ichi, m, store)
 ...
 /** Send Button 5 Message
   ...
   * @param {*} options
   */
   ichi.sendCatalog = async (jid , but = '' , text = '' , footer = '' , nama_owner = '', url_ad = '', nomer_owner = '', img , options = {}) =>{
   var messa = await prepareWAMessageMedia({ image: img }, { upload: ichi.waUploadToServer })
   var catalog = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
   "productMessage": {
   "product": {
   "productImage": messa.imageMessage,
   "productId": "4803777656393383",
   "title": but,
   "description": text,
   "footerText": footer, 
   "currencyCode": "IDR",
   ...
   "productImageCount": 1,
   "firstImageId": 1,
   "retailerId": nama_owner,
   "url": url_ad
   },
   "businessOwnerJid": nomer_owner+`@s.whatsapp.net`,
   }
   }), options)
   ichi.relayMessage(m.chat, catalog.message, { messageId: catalog.key.id })
   }
